...
export default function HelperVideoCallPage() {
  ...
  const helperName = currentUser?.profile?.name || "You"
      ...
      <main className="flex-1 p-8 flex items-center justify-center">
        <div className="text-center space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Thank you for supporting the seeker!</p>
          </div>
          <div className="bg-card border border-border rounded-2xl p-6 text-left">
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Helper</span>
                <span className="font-semibold text-foreground">{helperName}</span>
              </div>
              <div className="flex justify-between">
    ...
    <main className="flex-1 bg-black flex flex-col">
      <div className="flex-1 flex items-center justify-center relative gap-4 p-4">
        ...
        <div className="flex-1 rounded-3xl overflow-hidden shadow-2xl border-4 border-white/10">
          <VideoStream userName="Seeker" userInitial="S" isVideoOn={isVideoOn} />
        </div>

        {/* Local Video (Self) - Bottom Right */}
        <div className="absolute bottom-8 right-8 w-48 h-48 shadow-2xl">
          <VideoStream userName={helperName} userInitial={helperInitial} isLocal isVideoOn={isVideoOn} />
        </div>
